...
from transformers.models.qwen3_5.modeling_qwen3_5 import (
    Qwen3_5VisionAttention, Qwen3_5VisionModel, apply_rotary_pos_emb_vision)
...
class Qwen3_5VisionAttentionPatch(Qwen3_5VisionAttention):
    """
    ...
    def __init__(self, attention_module: Qwen3_5VisionAttention) -> None:
        """
    ...
    def forward(
        ...
        **kwargs,
        ...
        if max_seqlen_carrier is None:
            raise ValueError("max_seqlen_carrier must be provided.")
...
class Qwen3_5VisionModelPatch(Qwen3_5VisionModel):
    """
    Patched version of Qwen3_5VisionModel for ONNX export.

    This class provides a wrapper around the original Qwen3-5 vision transformer
    with custom blocks that are compatible with ONNX export.
    """

    def __init__(self, original_model: Qwen3_5VisionModel) -> None:
        """
        Initialize the patched vision transformer from original model.

        Args:
            original_model: Original Qwen3_5VisionModel instance
        """
        ...
        for block in self.blocks:
            if not isinstance(block.attn, Qwen3_5VisionAttention):
                raise TypeError(
                    f"Expected Qwen3_5VisionAttention, got {type(block.attn)}"
                )
            block.attn = Qwen3_5VisionAttentionPatch(block.attn)

    def fast_pos_embed_interpolate_optimized(self, grid_thw):
        """
        Optimized version of `fast_pos_embed_interpolate` in Qwen3_5VisionModel.
    ...
    def forward(
        ...
        max_seqlen_carrier: Optional[torch.Tensor] = None,
        fast_pos_embed_idx: Optional[torch.Tensor] = None,
        fast_pos_embed_weight: Optional[torch.Tensor] = None,
        **kwargs,
    ) -> torch.Tensor:
        """
        ...
        if cu_seqlens is None:
            raise ValueError("cu_seqlens must be provided.")
        if max_seqlen_carrier is None:
            raise ValueError("max_seqlen_carrier must be provided.")
        if fast_pos_embed_idx is None or fast_pos_embed_weight is None:
            raise ValueError(
                "fast_pos_embed_idx and fast_pos_embed_weight must be provided."
            )
...
def export_qwen3_5_visual(
    model: Qwen3_5VisionModelPatch,
    output_dir: str,
